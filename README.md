...
Main menu, *LONG-PRESS EACH BUTTON* to see its function
  ...
  col 4) Per-split row offset (OFF, NOVR = no overlap, 0 to 25, tap blue button for positive/negative offsets)
...
Joystick mode lets you shape your sound as you're playing, much like an Expressive E Touche SE would. Control 3 to 5 additional CCs at once by rolling one fingertip within a single pad on the low row. 10 dimensions of MPE! All CCs are reset to 0 upon releasing the low row, unless you first latch the CCs via a 2nd touch in the low row. Latching lets you play the new sound with both hands. The low row turns color to indicate the latching. Once the low row is completely untouched, the next touch un-latches.

  *Details: On Per-Split Settings, long-press the XYZ pad. Scroll past HLD (hold) and FDR (fader) to JOY (joystick). JOY is like HLD, but it reduces the range of X from 7 pads to 1, and moves the zero point from the center of the pad to the left edge. Scrolling down past X, Y and Z reveals the X' and Y' CCs which default to OFF. Setting X' to a CC moves the zero point back to the center of the pad. Roll to the right to send the X CC, and roll to the left to send the X' CC. X and X' can be set to match, e.g. both CC16. The Y' CC works similarly, roll up for Y and down for Y'.*

  *Scroll down below X' and Y' to find "X=0". This indicates that the X CC resets to 0 when the low row is untouched and unlatched. Swipe right to change it to "X=64" to reset to centered. X=64 makes the most sense when X' is OFF, because the reset value will correspond to the center of the pad, analogous to a physical joystick. Below "X=0" is "Y=0", which works similarly. X=64 and Y=64 also apply to the HLD mode.*
  ...
  *Whereas the X and X' CCs can match, and the Y and Y' CCs can match, it's best if no other low row CCs match. Otherwise a single touch will send two values for one CC, and the CC value will jump abruptly. As always, if any CC is sent by the low row and also by normal play (i.e. TIMBRE/Y or LOUDNESS/Z), the low row takes priority, and touching the low row temporarily disables sending the normal play CC.*
...
In the Preset display, Program Change messages are always sent on touch release, not new touch. This prevents a single swipe sending multiple PC messages. To send a Bank Select message instead, hold the blue dot in the lower left corner and swipe as usual.
...
You can back up various settings and/or share them with others via midi files. The 6 memories, the 3 custom light patterns, the 16 audience messages, microtonal data, and all settings combined. A memory or light pattern loads in under 2 seconds, doable on stage in between songs. Check the LinnStrument Community Wiki for export-request files and importable settings files.
...
* You can cycle through the edos by setting switch 1 to EDO+ and switch 2 to EDO- (you must select an edo first)
* Set the default layout to e.g. Bosanquet to stay in Bosanquet while you cycle
* The first few edos are pretty strange, so you may want to start with 19 or 22 (or possibly 15 or 17). Or 24 for middle easternn music.
...
The 9 scales in Global Settings columns 2-4 are now microtonal and change for each edo. The 3 custom light patterns work as usual.
  ...
  *Default scales: 3 of the 7 scales are blank but for the tonic, so that you can create your own scales on the note lights screen. The other 4 are MOS scales. They are chosen somewhat arbitrarily from many possibilities. The L/s ratio is at most 2:1. They use the brightest (sharpest) mode that includes the 5th.*

  *The 12edo scales are an exception. Had they been chosen similarly, they would have been minor pentatonic, whole tone, lydian, whole step/half-step diminished octotonic and Tcherepnin enneatonic. Instead it's the major and minor diatonic (5L2s) and pentatonic (2L3s) scales. Note that the major scale is simply a vertical column of white notes. This applies to all but a few of the smaller edos. Since the 5L2s scale is built into the coloring scheme and is so easy to find, it's rarely listed. Precedence has been given to scales that are harder to find.*

  *Scales can be interesting melodically (MOS), smooth harmonically, or just useful markers.*
  *Edos 15 and above often have a 12-note scale that can be condensed to a familiar 12-note layout.*
  ...
  * *16edo: 4L2s(3:2), 2L5s(3:2), 7L2s(2:1), 4L8s(2:1), also try Ls(:) and Ls(:)*
  * *17edo: 5L1s(3:2), 3L4s(3:2), 1L7s(3:2), 5L7s(2:1), also try Ls(:) and Ls(:)*
  * *18edo: 3L2s(4:3), 4L3s(3:2), 2L6s(3:2), 6L6s(2:1), also try Ls(:) and Ls(:)*
  * *19edo: 1L5s(4:3), 3L5s(3:2), 1L8s(3:2), 7L5s(2:1), also try 4L1s(4:3) and Ls(:)*
  * *, also try Ls(:) and Ls(:)*
  * *, also try Ls(:) and Ls(:)*
  * *, also try Ls(:) and Ls(:)*
  * *, also try Ls(:) and Ls(:)*
